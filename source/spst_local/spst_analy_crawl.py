...
boundmorpheme = ["은", "는", "이", "가", "을", "를", "로써", "계신","혹시","에서", "분", "에게서", "부터", "까지", "에게", "한테", "께", "와", "과", "의", "로서", "으로서", "로", "으로"] # 조사
...
def exceling():
    usersetf = pd.read_sql("SELECT * FROM FEELXL",conf)
    posl = []
    navl = []
    natl = []

    for x in range(len(usersetf['WORD'])): 
        if(usersetf['FEEL_RANGE'][x] == '기쁨' or usersetf['FEEL_RANGE'][x] == '  흥미' or usersetf['FEEL_RANGE'][x] == '놀람'):
            posl.append(usersetf['WORD'][x])
        if(usersetf['FEEL_RANGE'][x] == '혐오' or usersetf['FEEL_RANGE'][x] == '분노' or usersetf['FEEL_RANGE'][x] == '통증' or usersetf['FEEL_RANGE'][x] == '지루함' or usersetf['FEEL_RANGE'][x] == '공포'):
            navl.append(usersetf['WORD'][x])
        if(usersetf['FEEL_RANGE'][x] == '슬픔' or usersetf['FEEL_RANGE'][x] == '중성' or usersetf['FEEL_RANGE'][x] == '기타'):
            natl.append(usersetf['WORD'][x])

    feels = list(usersetf['FEEL_RANGE'])
    ...
    if(len(text1) != 0):
        for user in userlist:
            if(text1 == user):
                ...
                values = [user_profile["id"][0],str(int(user_profile["time"][0]))+"시",user_profile["user_topic"][0],user_profile["emotion"][0]]
